    ...
    void BuildDungeonUi() {
        ...
        DungeonMake(L"STATIC", L"AUTO PHÓ BẢN v4.4 • FIGHT: cứ 5 giây scan GMonster/HP • còn quái = đánh • hết quái = sang STEP",
    ...
    void RefreshDungeonProgressPanel(DWORD now = GetTickCount()) {
        ...
        if (preset && team.phase == cleanroute_dungeon::TeamPhase::Steps && team.stepIndex >= 0 && team.stepIndex < static_cast<int>(preset->steps.size())) {
            ...
            DungeonProgressAdd(L"Vị trí", L"M" + std::to_wstring(step.mapID) + L" • " + std::to_wstring(step.x) + L"," + std::to_wstring(step.y) +
