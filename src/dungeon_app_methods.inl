    ...
    void ResolveDungeonTeamBindings() {
        for (DungeonTeamRuntime& team : dungeonTeams_) {
            ...
            if (team.config.pids.size() == team.memberRoleIDs.size() && team.config.leaderPid != 0)
    ...
    void BuildDungeonUi() {
        ...
        DungeonMake(L"STATIC", L"AUTO PHÓ BẢN v4.0 • mỗi tổ đội 1–6 acc • nhiều tổ đội chạy độc lập",
        ...
        DungeonMake(L"STATIC", L"Bán <", SS_LEFT | SS_CENTERIMAGE, 132, 234, 42, 28, 0);
        dungeonSellThresholdEdit_ = DungeonMake(L"EDIT", L"40", WS_BORDER | ES_NUMBER | ES_CENTER,
                                                176, 234, 43, 28, IDC_DG_SELL_THRESHOLD);
        DungeonMake(L"BUTTON", L"LƯU", BS_PUSHBUTTON, 224, 234, 54, 28, IDC_DG_SAVE_THRESHOLD);
        DungeonMake(L"STATIC", L"Đội trưởng:", SS_LEFT | SS_CENTERIMAGE, 287, 234, 70, 28, 0);
        dungeonLeaderCombo_ = DungeonMake(WC_COMBOBOXW, L"", CBS_DROPDOWNLIST | WS_VSCROLL,
                                          359, 234, 160, 300, IDC_DG_LEADER);
        DungeonMake(L"STATIC", L"Phó bản:", SS_LEFT | SS_CENTERIMAGE, 527, 234, 58, 28, 0);
        dungeonPresetCombo_ = DungeonMake(WC_COMBOBOXW, L"", CBS_DROPDOWNLIST | WS_VSCROLL,
                                          587, 234, 195, 400, IDC_DG_PRESET);
        DungeonMake(L"STATIC", L"Lượt:", SS_LEFT | SS_CENTERIMAGE, 790, 234, 38, 28, 0);
        dungeonRunsEdit_ = DungeonMake(L"EDIT", L"1", WS_BORDER | ES_NUMBER | ES_CENTER,
                                       830, 234, 42, 28, IDC_DG_RUNS);
        DungeonMake(L"BUTTON", L"TẠO TỔ ĐỘI", BS_DEFPUSHBUTTON, 880, 234, 143, 28, IDC_DG_CREATE_TEAM);
        ...
        DungeonListColumn(dungeonTeamList_, 0, 70, L"Tổ đội");
        DungeonListColumn(dungeonTeamList_, 1, 150, L"Đội trưởng");
        DungeonListColumn(dungeonTeamList_, 2, 270, L"Thành viên");
        DungeonListColumn(dungeonTeamList_, 3, 190, L"Phó bản");
        DungeonListColumn(dungeonTeamList_, 4, 65, L"Lượt");
        DungeonListColumn(dungeonTeamList_, 5, 85, L"State");
        DungeonListColumn(dungeonTeamList_, 6, 165, L"Tiến độ");
    ...
    int SelectedDungeonTeamIndex() const {
        return dungeonTeamList_ ? ListView_GetNextItem(dungeonTeamList_, -1, LVNI_SELECTED) : -1;
    }
    ...
    void RefreshDungeonTeamList(int select = -1) {
        ...
        for (std::size_t i = 0; i < dungeonTeams_.size(); ++i) {
            ...
            std::wstring id = L"ĐỘI " + std::to_wstring(team.config.id);
            LVITEMW item{};
            item.mask = LVIF_TEXT;
            item.iItem = static_cast<int>(i);
            item.pszText = id.data();
            ListView_InsertItem(dungeonTeamList_, &item);
            std::wstring leader = team.leaderRoleID > 0 ? L"Role " + std::to_wstring(team.leaderRoleID) : L"?";
            if (team.config.leaderPid != 0) {
                if (Account* account = AccountByPid(team.config.leaderPid)) leader = AccountTag(*account);
            }
            ListView_SetItemText(dungeonTeamList_, static_cast<int>(i), 1, leader.data());
            std::wstring members = DungeonTeamMembers(team);
            ListView_SetItemText(dungeonTeamList_, static_cast<int>(i), 2, members.data());
            std::wstring presetName = preset ? preset->name : L"?";
            ListView_SetItemText(dungeonTeamList_, static_cast<int>(i), 3, presetName.data());
            std::wstring runs = std::to_wstring(team.runIndex) + L"/" + std::to_wstring(team.config.runs);
            ListView_SetItemText(dungeonTeamList_, static_cast<int>(i), 4, runs.data());
            std::wstring state = cleanroute_dungeon::TeamStateLabel(team.state);
            ListView_SetItemText(dungeonTeamList_, static_cast<int>(i), 5, state.data());
            ListView_SetItemText(dungeonTeamList_, static_cast<int>(i), 6, team.status.data());
        }
        if (select < 0 && !dungeonTeams_.empty())
            select = std::clamp(SelectedDungeonTeamIndex(), 0, static_cast<int>(dungeonTeams_.size()) - 1);
        if (select >= 0 && select < static_cast<int>(dungeonTeams_.size())) {
            ListView_SetItemState(dungeonTeamList_, select, LVIS_SELECTED | LVIS_FOCUSED,
                                  LVIS_SELECTED | LVIS_FOCUSED);
            ListView_EnsureVisible(dungeonTeamList_, select, FALSE);
        }
    }
    ...
    bool TickDungeonFight(DungeonTeamRuntime& team, const cleanroute_dungeon::Step& step,
        ...
        if (!allOn) {
            if (!team.dueTick || now >= team.dueTick) {
                for (Account* account : members) {
                    if (account->snapshot.autoFight) continue;
                    Response response{};
                    std::wstring callError;
                    if (!account->bridge.Call(Command::StartAutoFight, 0, 0, 0, response, callError, 1200)) {
                        team.status = L"chờ bật AutoFight • " + callError;
                        continue;
                    }
                }
                team.dueTick = now + 1200;
            }
            team.status = taskSnapshotFresh
                ? L"AUTO-ĐÁNH • TASK snapshot OK • chờ participant AutoFight ON"
                : L"AUTO-ĐÁNH • TASK chưa khả dụng • chờ participant AutoFight ON";
            return false;
        }
    ...
    void TickDungeonTeam(DungeonTeamRuntime& team, DWORD now) {
        ...
        if (team.phase == cleanroute_dungeon::TeamPhase::Steps) {
            if (team.stepIndex >= static_cast<int>(preset->steps.size())) {
                for (std::uint32_t pid : team.config.pids) {
                    if (Account* account = AccountByPid(pid)) {
                        Response response{};
                        std::wstring callError;
                        (void)account->bridge.Call(Command::StopAutoFight, 0, 0, 0, response, callError, 800);
                    }
                }
                team.phase = cleanroute_dungeon::TeamPhase::WaitExit;
                team.phaseTick = now;
                team.status = L"HẾT STEP • chờ server rời Map " + std::to_wstring(preset->dungeonMap);
                return;
            }
            ...
            if (step.kind == cleanroute_dungeon::StepKind::Fight) {
                if (TickDungeonFight(team, step, now, error)) {
                    for (Account* account : stepMembers) {
                        Response response{};
                        std::wstring callError;
                        (void)account->bridge.Call(Command::StopAutoFight, 0, 0, 0, response, callError, 800);
    ...
    void OpenDungeonEditor() {
        ...
        const std::wstring title = L"CẤU HÌNH AUTO PHÓ BẢN v4.0 — " +
