...
public:
    ...
    std::vector<KillEvent> Observe(const std::vector<MonsterObservation>& scan,
        ...
        for (const MonsterObservation& monster : scan) {
            ...
            const bool matched = matchAnyVerified || matching != rules.end();
            if (!matched) {
                if (diagnostics) {
                    diagnostics->push_back({CountSkipReason::WrongGroup, monster.roleID,
                                            monster.resID, monster.name});
                }
                continue;
            }
            if (!InRadius(monster, centerX, centerY, radius)) {
                if (diagnostics) {
                    diagnostics->push_back({CountSkipReason::OutsideRadius, monster.roleID,
                                            monster.resID, monster.name});
                }
                continue;
            }
            ...
            if (aliveNow) {
                // Seeing the same dynamic RoleID alive after a counted death explicitly rearms one new life.
            ...
            // Fail closed: first-seen corpses never become inferred kills.
            if (deadNow && !life.seenAlive && diagnostics) {
                const bool roleChanged = std::any_of(lives_.begin(), lives_.end(), [&](const auto& item) {
                    ...
                    return !monster.name.empty() && !other.name.empty() &&
                           EqualFolded(monster.name, other.name);
                });
                diagnostics->push_back({roleChanged ? CountSkipReason::RoleIdChanged
                                                    : CountSkipReason::NotSeenAlive,
...
inline bool ValidateStep(const Step& step, std::wstring& error) {
    ...
    if (step.kind == StepKind::Fight && step.requiredKills <= 0) {
        error = L"STEP đánh quái phải có Kill > 0";
        return false;
    }
...
inline const wchar_t* CountSkipReasonText(CountSkipReason reason) {
    switch (reason) {
        case CountSkipReason::WrongGroup: return L"sai nhóm/filter";
        case CountSkipReason::OutsideRadius: return L"ngoài radius";
        case CountSkipReason::NotSeenAlive: return L"chưa từng thấy sống";
        case CountSkipReason::RoleIdChanged: return L"nghi RoleID đã đổi";
    }
    return L"không rõ";
}
