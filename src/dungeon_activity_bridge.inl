...
void DungeonActivityAppendTokens(const std::wstring& raw, std::vector<std::wstring>& tokens) {
    ...
    while (start <= raw.size()) {
        const std::size_t slash = raw.find(L'/', start);
        std::wstring token = DungeonActivityStripMarkup(raw.substr(start, slash == std::wstring::npos ? std::wstring::npos : slash - start));
