...
constexpr wchar_t kTitle[] = L"AUTO Thần Long đa tính năng Pro v4.1";
    ...
    void OnListNotification(const NMHDR* hdr) {
        ...
        if (hdr->hwndFrom == dungeonTeamList_ && hdr->code == LVN_ITEMCHANGED) {
            ...
            if ((n->uChanged & LVIF_STATE) != 0 && (n->uNewState & LVIS_SELECTED) != 0) {
                ...
                if (n->iItem >= 0 && n->iItem < static_cast<int>(dungeonTeams_.size()) && dungeonStatus_)
                    SetText(dungeonStatus_, L"ĐỘI " + std::to_wstring(dungeonTeams_[static_cast<std::size_t>(n->iItem)].config.id) +
                                            L" • " + dungeonTeams_[static_cast<std::size_t>(n->iItem)].status);
            }
            return;
        }
