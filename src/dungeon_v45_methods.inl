    ...
    void EnsureDungeonManagerWindow() {
        ...
        if (!registered) {
            WNDCLASSEXW wc{}; wc.cbSize = sizeof(wc); wc.hInstance = GetModuleHandleW(nullptr);
            wc.lpfnWndProc = DungeonManagerWndProc; wc.lpszClassName = L"ThanLongDungeonManagerV45";
            wc.hCursor = LoadCursorW(nullptr, IDC_ARROW); wc.hbrBackground = reinterpret_cast<HBRUSH>(COLOR_WINDOW + 1);
            registered = RegisterClassExW(&wc) != 0 || GetLastError() == ERROR_CLASS_ALREADY_EXISTS;
        }
        if (!registered) return;
        dungeonManagerWindow_ = CreateWindowExW(WS_EX_TOOLWINDOW, L"ThanLongDungeonManagerV45",
    ...
    void RefreshDungeonManagerFromSelection() {
        ...
        for (const auto& ptr : accounts_) {
            ...
            std::wstring label = std::wstring(ptr->snapshot.characterName) + L" • " + std::to_wstring(roleID);
            LVITEMW item{}; item.mask = LVIF_TEXT | LVIF_PARAM; item.iItem = row; item.lParam = roleID; item.pszText = label.data();
            ListView_InsertItem(dungeonManagerMembers_, &item);
            std::wstring pid = std::to_wstring(ptr->game.pid); ListView_SetItemText(dungeonManagerMembers_, row, 1, pid.data());
            ListView_SetCheckState(dungeonManagerMembers_, row,
                std::find(team.memberRoleIDs.begin(), team.memberRoleIDs.end(), roleID) != team.memberRoleIDs.end());
    ...
    void DungeonV45UpdateTeamFromManager() {
        ...
        std::vector<std::int32_t> roles; std::vector<std::uint32_t> pids;
        const int rows = ListView_GetItemCount(dungeonManagerMembers_);
        for (int row = 0; row < rows; ++row) {
            if (!ListView_GetCheckState(dungeonManagerMembers_, row)) continue;
            LVITEMW item{}; item.mask = LVIF_PARAM; item.iItem = row;
            if (!ListView_GetItem(dungeonManagerMembers_, &item)) continue;
            const auto roleID = static_cast<std::int32_t>(item.lParam);
            for (const auto& ptr : accounts_) {
                if (ptr && ptr->snapshotValid && (ptr->snapshot.validMask & ValidIdentity) && ptr->snapshot.roleID == roleID) {
                    roles.push_back(roleID); pids.push_back(ptr->game.pid); break;
                }
            }
        }
