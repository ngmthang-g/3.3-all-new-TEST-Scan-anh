...
need((ROOT / 'VERSION.txt').read_text(encoding='utf-8').strip() == '4.3', 'VERSION.txt must be 4.3')
need('project(ThanLongItemConsolidator VERSION 4.3' in CM, 'CMake version must be 4.3')
need('FILEVERSION 4,3,0,0' in RC and 'PRODUCTVERSION 4,3,0,0' in RC, 'VERSIONINFO must be 4.3')
need('AUTO Thần Long đa tính năng Pro v4.3' in C, 'visible product title v4.3 missing')
need('AUTO_Than_Long_da_tinh_nang_Pro_v4.3.exe' in RC and
     'AUTO-Than-Long-da-tinh-nang-Pro-v4.3-win-x64.zip' in W, 'v4.3 artifact names missing')
