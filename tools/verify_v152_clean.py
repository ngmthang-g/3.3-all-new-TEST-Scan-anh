...
need((ROOT / 'VERSION.txt').read_text(encoding='utf-8').strip() == '4.1', 'VERSION.txt must be 4.1')
need('project(ThanLongItemConsolidator VERSION 4.1' in CM, 'CMake version must be 4.1')
need('FILEVERSION 4,1,0,0' in RC and 'PRODUCTVERSION 4,1,0,0' in RC, 'VERSIONINFO must be 4.1')
need('AUTO Thần Long đa tính năng Pro v4.1' in C, 'visible product title v4.1 missing')
need('AUTO_Than_Long_da_tinh_nang_Pro_v4.1.exe' in RC and
     'AUTO-Than-Long-da-tinh-nang-Pro-v4.1-win-x64.zip' in W, 'v4.1 artifact names missing')
need('kProtocolVersion = 0x00040100u' in P, 'EXE/DLL protocol must be v4.1')
...
for forbidden in [
    'TravelSemantic::DaiLy', 'DaiLy =', 'ReservedDaiLy', 'selectionID=200001',
...
print('AUTO Than Long da tinh nang Pro v4.1 verifier: PASS')
