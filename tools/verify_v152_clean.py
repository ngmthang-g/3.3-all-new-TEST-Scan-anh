...
need((ROOT / 'VERSION.txt').read_text(encoding='utf-8').strip() == '4.4', 'VERSION.txt must be 4.4')
need('project(ThanLongItemConsolidator VERSION 4.4' in CM, 'CMake version must be 4.4')
need('FILEVERSION 4,4,0,0' in RC and 'PRODUCTVERSION 4,4,0,0' in RC, 'VERSIONINFO must be 4.4')
need('AUTO Thần Long đa tính năng Pro v4.4' in C, 'visible product title v4.4 missing')
need('AUTO_Than_Long_da_tinh_nang_Pro_v4.4.exe' in RC and
     'AUTO-Than-Long-da-tinh-nang-Pro-v4.4-win-x64.zip' in W, 'v4.4 artifact names missing')
...
need('L"TucCau_1"' in DP and 'L"Tiêu diệt túc cầu",103,1312,3072' in DP, 'canonical Túc Cầu preset missing')
need('dungeon_logic_tests' in CM and 'dungeon_progress_logic_tests' not in CM, 'old kill-progress test must be removed from CMake')
for forbidden in ['requiredKills', 'DeathTracker', 'CountDiagnostic', 'CountSkipReason', 'team.kills', 'dungeon_progress_logic']:
    need(forbidden not in D + DA + CM, f'old dungeon kill-count logic remains: {forbidden}')
need('Elapsed(now, team.lastScanTick, 5000)' in DA and 'GMonster sống=0' in DA and
     'GMonster sống=' in DA and 'SCAN FAIL • tiếp tục đánh, không được coi là hết quái' in DA,
     'v4.4 5-second GMonster presence completion contract missing')
...
# v4.4 task-board observer + scanner UI contracts.
need('ReadDungeonProgress = 27' in P and 'DungeonProgressSnapshot' in P, 'task progress protocol snapshot missing')
need('GetDoingTasks' in B and 'DungeonReadTaskParameters' in B and 'DungeonFindMethodFlexible' in B and
     'get_Values' in B and 'TaskID' in B and 'Parameters' in B,
     'robust semantic task snapshot reader missing')
need('Command::ReadDungeonProgress' in DA and 'TASK observer' in DA,
     'TASK must remain observer/diagnostic only in v4.4')
need('dungeonProgressList_' in C and 'dungeonTaskList_' in C and 'dungeonScanList_' in C,
     'v4.4 progress/task/monster UI lists missing')
need('Tên Monster' in DA and 'HP / MaxHP' in DA and 'Parameter key=value' in DA,
     'v0.6.2-style monster HP/name + task parameter diagnostics missing')
need('DungeonMonsterCatalog' in DA and 'LƯU MONSTER ĐÃ CHỌN' in DA and 'PopulateDungeonMonsterCombo' in DA,
     'v0.6.2-style saved monster name/ResID catalog missing')
need('HighlightDungeonCurrentStep' in DA and 'Timeout còn' in DA,
     'current-step highlight/countdown progress UI missing')
...
print('AUTO Than Long da tinh nang Pro v4.4 verifier: PASS')
