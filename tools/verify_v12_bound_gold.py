...
need(VERSION=='1.2', f'VERSION must be 1.2, got {VERSION!r}')
need('VERSION 1.2.0' in CMAKE, 'CMake version not 1.2.0')
need('v1.2 • AUTO PK + TELEGRAM' in CONTROLLER, 'v1.2 title missing')
...
for marker in ['FunnyAutoTrainOffText', '20u * 60u * 1000u', 'FunnySellText', 'FunnyDeathBurstText',
    ...
    need(marker in CONTROLLER or marker in LOGIC, 'requested Telegram marker missing: '+marker)
...
need('kCurrencyUnitsPerGold = 10000LL' in LOGIC, 'game currency scale must be 10000 raw units per gold')
need('WholeGoldFromRaw' in CONTROLLER and 'WholeGoldDeltaFromRaw' in CONTROLLER, 'whole-gold conversion missing from Telegram reports')
need('boundMoneyBaselineTime = LocalHourMinuteText()' in CONTROLLER, 'bound-gold baseline time not captured')
need('Keep the bound-gold reference fixed' in CONTROLLER, '24h must not reset bound-gold reference')
need('PhraseIndex(seed, 5)' in CONTROLLER, 'currency taunt pool must contain five variants')
need('Vàng khóa: ' in CONTROLLER and 'so với mốc ' in CONTROLLER, 'short bound-gold milestone text missing')
# Normal Money may still be read by the bridge for compatibility, but Telegram output must not render it.
currency_fn=function_body(CONTROLLER, '    static std::wstring FunnyCurrencyText(')
need(currency_fn is not None and 'watch.money' not in currency_fn, 'milestone must not report normal Money')
if errors:
    for e in errors: print('FAIL:',e)
    sys.exit(1)
print('verify_v12_bound_gold PASS')
