...
# Compile-safe ROI correction + user-approved dynamic click-list cleanup.
...
for forbidden in ['Sleep(', 'kMaxClickSteps', 'phải có đúng 20 tọa click', 'BỘ LỌC cố định 20 CLICK']:
    if forbidden in impl:
        raise SystemExit(f'FILTER V4 forbidden legacy behavior: {forbidden}')

wrapper = '''#include "image_scan_test.h"\n\n// TEST FILTER v4 • 1 PrintWindow/probe + 3 independent ROI + non-blocking timer/state + dynamic click list.\n#include "image_scan_filter_v4.inl"\n'''
Path('src/image_scan_test.cpp').write_text(wrapper, encoding='utf-8')

print('TEST FILTER v4 finalize PASS • default 20 + unlimited add/remove • 1 capture/probe • 3 ROI • adaptive timer/state • no Sleep')
